...
class PlatformUseCase(UseCase):
    # Class for general platorm.sh interview - general use-case logic
    ...
    def list_containers(self):
        ...
        for i in lxc.list_containers():
            ...
            _ret = self.give_container_info(container=_c, name=i, _ret=_ret, short=True)
        return _ret
    ...
    def destroy_container(self, container_name: str):
        ...
        try:
            ...
            if not lxc.Container(name=container_name).destroy():
                raise HTTPException(
                    status_code=409, detail=f"Container failed to destroy"
                )
